...
const dataDir = path.join(root, process.env.LEAN_EVENT_DATA_DIR ?? ".lean-event-data");
...
async function main() {
  ...
  const registryLines = [
    "# Lean Event — Registro accessi clienti",
    "",
  ...
  if (process.env.LEAN_EVENT_SKIP_VERCEL_SYNC === "1") {
    console.log("Sync Vercel saltato (LEAN_EVENT_SKIP_VERCEL_SYNC=1).");
    console.log("Manuale: npm run lean-event:sync-vercel");
    return;
  }

  try {
    const { syncLeanEventTenantsToVercel } = await import(
      "./sync-lean-event-vercel-env.mjs"
    );
    await syncLeanEventTenantsToVercel({ quiet: false });
  } catch (error) {
    console.warn("");
    console.warn("Sync Vercel automatico non eseguito:", error.message);
    console.warn("Fallback: npm run lean-event:vercel-env → incolla su Vercel");
    console.warn("Oppure: vercel link && npm run lean-event:sync-vercel");
    console.warn("");
  }
}
